...
patro_dict = {}
#Lista para guardar los objetos de escuderias
escObj = []
    ...
    def __init__ (self,venprincipal):
        ...
        if escuderias_dict == {}:
            ...
            for i in range(1,esc_cont):
                ...
                escObj+= [i]
                print(escObj)
                i.graficar(Baseframe,row_cont,col_cont)
                col_cont+=1
                        

        crearEscuderias = Button(self.escuderias,text='Crear Escuderias',command=lambda:Crear_Escuderia(self.escuderias),fg='white',bg='blue', font=('Agency FB',12))
...
class Crear_Escuderia:
    ...
    def __init__(self,escuderias):
        ...
        GuardarEscuderia = Button(self.vencrear,text="Guardar Escuderia",fg='white',bg='blue',font=('Agency FB',12),command=lambda:save(self)).place(x=400,y=500)

        SalirBtn = Button(self.vencrear,text="Salir",fg='white',bg='blue',font=('Agency FB',12),command=lambda:salir(self,self.escuderias))
        SalirBtn.place(x=100,y=500)

        def salir(self,escuderias):
            escuderias.deiconify()
            self.vencrear.destroy()
        ...
        def save(self):
            ...
            for i in info: #ciclo for para evaluar si falta algún dato
                if i == '': #si algun dato no se completó, señala el error
                    ...
                    saved_info =  NEscuderia (name,logo,Ubic,Patro)
...
class NEscuderia:
    ...
    def __init__(self,name,logo,Ubic,Patro):
        self.name = name
        self.logo = logo
        self.Ubic = Ubic
        self.Patro = Patro

    def graficar(self,ventana,ROW,COL):
        frame = Frame(ventana,bg="light blue")
        frame.grid(row=ROW,column=COL,ipadx = 5)
        NameLb = Label(frame,text='Escuderia '+self.name,bg = "orange").grid(row=ROW,column=COL)
        ROW+=1
        LogoLb = Label(frame,text='Logo:'+self.logo,bg = "light blue").grid(row=ROW,column=COL)
        ROW+=1
        UbicLb = Label(frame,text='Ubicacion:'+self.Ubic,bg = "light blue").grid(row=ROW,column=COL)
        ROW+=1
        PatroLb = Label(frame,text='Patrocina:'+self.Patro,bg = "light blue").grid(row=ROW,column=COL)
...
class VenPilotos:
    ...
    def __init__(self,venprincipal):
        ...
        self.venpiloto.minsize(900,627)
        self.venpiloto.resizable(width=NO,height=NO)
        self.fondo1=PhotoImage(file=("img/pilot image.png"))
        self.lblfondo1 = Label(self.venpiloto,image=self.fondo1).place(x=0,y=0)

        RegPilBoton = Button(self.venpiloto,text='Registrar Pilotos',command=lambda:Registrar_Pilotos(self.venpiloto),fg='white',bg='blue', font=('Agency FB',12))
        RegPilBoton.place(x=680,y=500)

        SalirBtn = Button(self.venpiloto,text="Volver",fg='white',bg='blue',font=('Agency FB',12),command=lambda:self.salir(venprincipal))
        SalirBtn.place(x=100,y=500)

    def salir(self,venprincipal):
        venprincipal.deiconify()
        self.venpiloto.destroy()


class Registrar_Pilotos:
    
    def __init__(self,venpiloto):
        self.venpiloto = venpiloto
        ...
        """
        GuardarEscuderia = Button(self.venregpiloto,text="Guardar Escuderia",fg='white',bg='blue',font=('Agency FB',12),command=lambda:save(self)).place(x=400,y=500)

        SalirBtn = Button(self.venregpiloto,text="Salir",fg='white',bg='blue',font=('Agency FB',12),command=lambda:salir(self,self.venpiloto))
        SalirBtn.place(x=100,y=500)

        def salir(self,venpiloto):
            venpiloto.deiconify()
            self.venregpiloto.destroy()
